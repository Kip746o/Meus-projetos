...
/* Definição de arrays para representar os navios:
   ...
   em duas orientações possíveis: horizontal e vertical.
   O primeiro array representa a orientação horizontal e o segundo array, a vertical.
   Exemplo:
   - ship[0][0]: representa um navio horizontal de tamanho 2 com as coordenadas [1, 5].
   - ship[1][2]: representa um navio vertical de tamanho 3 com as coordenadas [6, 7, 8, 10].
*/
var ship = [
    [[1, 5], [1, 2, 5], [1, 2, 3, 5], [1, 2, 3, 4, 5]], // Orientação horizontal dos navios
    [[6, 10], [6, 7, 10], [6, 7, 8, 10], [6, 7, 8, 9, 10]] // Orientação vertical dos navios
 ];
 
 /* Informação usada para desenhar os navios afundados */
 /* Definição semelhante à variável 'ship', mas contendo as coordenadas para representar
    a versão dos navios quando eles são afundados (destruídos no jogo).
    Esses arrays indicam os quadrados que mostram visualmente que o navio foi destruído.
    Exemplo:
    - dead[0][1]: representa um navio horizontal de tamanho 3 afundado com as coordenadas [201, 202, 203].
    - dead[1][2]: representa um navio vertical de tamanho 3 afundado com as coordenadas [204, 205, 206].
 */
 var dead = [
    [[201, 203], [201, 202, 203], [201, 202, 202, 203], [201, 202, 202, 202, 203]], // Navios horizontais afundados
    [[204, 206], [204, 205, 206], [204, 205, 205, 206], [204, 205, 205, 205, 206]]  // Navios verticais afundados
 ];
 
 /* Descrição dos navios */
 /* Array com a descrição dos tipos de navios que os jogadores podem posicionar no tabuleiro.
    Cada elemento do array contém as seguintes informações:
    - O nome do navio
    - O comprimento do navio (número de quadrados que ele ocupa)
    - A quantidade desse tipo de navio disponível no jogo
    Exemplo:
    - shiptypes[0]: ["Rebocador", 2, 4], representa um "Rebocador" de tamanho 2, com 4 unidades disponíveis.
 */
 var shiptypes = [
    ["Rebocador", 2, 3], // Navio de tamanho 2, 3 unidades
    ["Contratopedeiro", 3, 2], // Navio de tamanho 3, 2 unidades
    ["Cruzador", 4, 2], // Navio de tamanho 4, 2 unidades
    ["Porta-avioes", 5, 1] // Navio de tamanho 5, 1 unidade
 ];
 
 var gridx = 10, gridy = 10;  /* Dimensões da grade do jogo: 16x16 quadrados. Cada jogador tem um tabuleiro de 16x16. */
 
 var player1 = [], player2 = []; /* Arrays que representarão as grades (tabuleiros) dos jogadores. */
 
 var player1Ships = [], player2Ships = []; /* Arrays que armazenarão as informações sobre os navios posicionados por cada jogador. */
 
 var player1Lives = 0, player2Lives = 0; /* Contadores de vidas (ou navios restantes) para cada jogador. */
 
 var currentPlayer = 1; /* Variável que armazena qual jogador está realizando a jogada atualmente. 
                           Pode ser 1 (jogador 1) ou 2 (jogador 2). */
...
var audioFogo = new Audio("./Som/Fogo.mp3"); //Atribui audio de Fogo para a variavel, que vai ser usado depois
 ...
 var statusMsg = ""; /* Variável que armazenará as mensagens de status do jogo, como o resultado de jogadas, quem venceu, etc. */
 
 /* Função para precarregar as imagens */
 /* Esta variável 'preloaded' será um array que armazenará as imagens que serão usadas no jogo.
    As imagens são carregadas antecipadamente para evitar atrasos durante a execução do jogo.
 */
 var preloaded = [];
     ...
     if (currentPlayer === 1) {
       ...
       if (player2[y][x][0] < 100) { // Verifica se a célula atacada não é água (número < 100 indica navio)
         ...
         if (--player2Ships[shipNo][1] === 0) { // Se a vida do navio chegou a zero, ele afundou
           ...
           if (--player2Lives === 0) {
             const player1Name = localStorage.getItem('player1Name') || 'Jogador 1';
             alert(`${player1Name}` + " venceu!"); // Exibe mensagem de vitória
     ...
     // Verifica se é a vez do Jogador 2
       ...
       if (player1[y][x][0] < 100) { // Verifica se a célula atacada não é água (número < 100 indica navio)
         ...
         if (--player1Ships[shipNo][1] === 0) { // Se a vida do navio chegou a zero, ele afundou
           ...
           audioExplosao.play(); // Da play no aúdio
           ...
           if (--player1Lives === 0) {
             const player2Name = localStorage.getItem('player2Name') || 'Jogador 2';
             alert(`${player2Name}` + " venceu!"); // Exibe mensagem de vitória
             currentPlayer = 0; // Define currentPlayer como 0 para indicar o fim do jogo
           }
         }
         ...
         currentPlayer = 1; // Alterna o turno para o Jogador 1
         audioAgua.play(); // Da play no aúdio
       }
     }
